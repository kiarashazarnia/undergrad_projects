 ...
 */
int add_food(food* food_list); // non sorted


int _add_food(food* food_list, food* new_food);
/*
...
int print_customer(customer* customers, char username[]);
 ...
 */
int add_order(Akbar_Joojeh* akbar_joojeh);


int _add_order(Akbar_Joojeh* akbar_joojeh, order* new_order);

/*
 * remover a order by its number
 *      return 1 if removed successfully, otherwise return 0
 */
int remove_order(order* , int );
...
int remove_food(food* food_list, char* food_name); // to remove a food by it's name
#endif